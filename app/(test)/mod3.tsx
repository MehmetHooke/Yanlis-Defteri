...
import { getMod3RetentionQuestions } from "@/src/services/test.service";
...
export default function TestMod3Screen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const qs = await getMod3RetentionQuestions({
          take: 5,
          poolLimit: 200,
          minDays: 7, // <- kalıcılık eşiği: bunu senin cevabına göre netleştireceğiz
        });
        setQuestions(qs);
        setI(0);
        setSolvedCount(0);
