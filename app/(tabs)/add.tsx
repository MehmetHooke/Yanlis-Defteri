...
import { addQuestion } from "@/src/services/question.service";
...
export default function AddScreen() {
    ...
    const onSave = async () => {
        ...
        try {
            ...
            // Home'a dön (liste yenileniyor çünkü focus effect)
            router.replace("/(tabs)");
        } catch (e: any) {
    ...
    return (
        <View className="flex-1 bg-black p-6 pt-14">
            ...
            <View className="flex-row gap-3 mb-4">
                <Pressable onPress={pickFromGallery} className="flex-1 rounded-xl bg-white px-4 py-3 items-center">
            ...
            {imageUri ? (
                <Image source={{ uri: imageUri }} className="w-full h-56 rounded-2xl mb-4" resizeMode="cover" />
            ) : (
